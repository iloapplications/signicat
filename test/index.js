...
const pubKeysResponse = { keys: [{ "kty": "RSA", "e": "AQAB", "use": "sig", "kid": "any.oidc-signature-preprod.test.jwk.v.1", "alg": "RS256", "n": "px6mGAGqVTcf8SNBzGF5ZzMQem8QH2wXO1xXEgwQAsBCcVvlpliIj1gkPDux36DYAgdUYy1wM7VhW6FHNhT1yCA7aYteUKB9hKAai3wzQNoUXPHQlKQsWRgTboFRQrkKzPgHHIp8IwZxBFzjCp9W9gdQ_LIQyCyjxoRTR0yg21HB1SC2bh91L2K689IpS9qcb7KBjizVmGqwRCgWtA1lBOKEpgrhPeHnSLcvRWG97ePR5MfmzftWxRftWIlDaIWV_3cnn8WsXH2Qtg4cq5FGBdS30SWHTpYNRuLYfvttivR1uZmx8fnnYEfy3L7lxHbWuVbdkySofQ7yvJWX56GGJw" }, { "kty": "RSA", "e": "AQAB", "use": "enc", "kid": "any.oidc-encryption-preprod.test.jwk.v.1", "alg": "RSA-OAEP", "n": "ou9ZQ_e0JSMhOA3fSwzH4h9OHgS8xLbtScHUlQEq9XWRw0i5ZefGWEUCeWJgehxuRMumPdm5_csfSnJLJom3c5cEnloXB53ZFEa6qJ7AEHnSjdMxnIkzcq_4ICQg69fwTac1ZCjxhCraUs6G9LE8b9gN-EHmd8MXuLRxZUkjlgiQKb-XhfDaDA7rd7KMczyxrieZT3q5lk1fjw2V_o_jasowLo8i7s8Wa4S7BAg1ZFv2-oc8PcobbJLsAAIxg3PEn0nDIvNcs6cjjYje2_TrrXMmis2TJquQhLOHjx_yQdzQNfzxC5_GwOZPBKZR1gH1-QxlW7q8jevC2-f_-7FlHw" }] };
...
const apiUrl = 'https://preprod.signicat.com/oidc/';

describe('Signicat', () => {
  describe('Constructor', () => {
    it('Should return an error if requestPromise not configured', () => {
      delete config.requestPromise;
      expect(() => new Signicat(config)).to.throw(Error, /requestPromise/);
    });
  ...
  describe('Get Authorization endpoint request', () => {
    ...
    it('Should return a valid authorization URL with signicat Full-flow example parameters', async () => {
      const resultUrl = apiUrl + 'authorize' +
        '?client_id=demo-preprod' +
        '&response_type=code' +
  ...
  describe('POST access token request', () => {
    ...
    it('Should return a valid access_token', async () => {
      sinon.stub(config, 'requestPromise').resolves(postAccessTokenSuccessResponse);
      const result = await new Signicat(config).postAccessToken(tokenParams);
      expect(result).to.equal(postAccessTokenSuccessResponse);
    });
  });
  ...
  describe('Get access token with full Message-level Encryption', () => {
    ...
    it('Should throw an error if can not verify id token signature', () => {
      ...
      config.FTN = true;
      config.privateJwk = privateJwk;
      sinon.stub(config, 'requestPromise')
        .onCall(0).resolves(postAccessTokenSuccessResponse)
        .onCall(1).resolves(pubKeysResponse);
  ...
  describe('Get User info with full Message-level encryption', () => {
    it('Should decrypt and verify signature from JWE userinfo token', async () => {
      ...
      config.privateJwk = privateJwk;
      sinon.stub(config, 'requestPromise')
        .onCall(0).resolves(userInfoResponse)
        .onCall(1).resolves(pubKeysResponse);
